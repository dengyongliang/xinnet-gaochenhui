...
//页签
$(function(){
    $(".header3-navcontent .tab1 li").bind("mouseenter", function () {
            var index = $(this).index();
			var divs = $(".header3-navcontent .content1 > div");
			$(this).parent().children("li").attr("class", "");
			$(this).attr("class", "on").show(); //设置当前选中项为选中样式
			divs.hide();//隐藏所有选中项内容
			divs.eq(index).show(); //显示选中项对应内容
        });
    // 解决方案
    	$(".header3-navcontent .tab_jjfa li").bind("mouseenter", function () {
            var index = $(this).index();
			var divs = $(".header3-navcontent .content_jjfa > div");
			$(this).parent().children("li").attr("class", "");
			$(this).attr("class", "on").show(); //设置当前选中项为选中样式
			divs.hide();//隐藏所有选中项内容
			divs.eq(index).show(); //显示选中项对应内容
        ...
        // end
		$(".header3-navcontent .tab2 li").bind("mouseenter", function () {
            var index = $(this).index();
			var divs = $(".header3-navcontent .content2 > div");
			$(this).parent().children("li").attr("class", "");
			$(this).attr("class", "on").show(); //设置当前选中项为选中样式
			divs.hide();//隐藏所有选中项内容
			divs.eq(index).show(); //显示选中项对应内容
        });
		$(".header3-navcontent .tab3_new li").bind("mouseenter", function () {
            var index = $(this).index();
			var divs = $(".header3-navcontent .content3 > div");
			$(this).parent().children("li").attr("class", "");
			$(this).attr("class", "on").show(); //设置当前选中项为选中样式
			divs.hide();//隐藏所有选中项内容
			divs.eq(index).show(); //显示选中项对应内容
        });
		$(".header3-navcontent .tab4 li").bind("mouseenter", function () {
            var index = $(this).index();
			var divs = $(".header3-navcontent .content4 > div");
			$(this).parent().children("li").attr("class", "");
			$(this).attr("class", "on").show(); //设置当前选中项为选中样式
			divs.hide();//隐藏所有选中项内容
			divs.eq(index).show(); //显示选中项对应内容
        });
		$(".header3-navcontent .tab5 li").bind("mouseenter", function () {
            var index = $(this).index();
			var divs = $(".header3-navcontent .content5 > div");
			$(this).parent().children("li").attr("class", "");
			$(this).attr("class", "on").show(); //设置当前选中项为选中样式
			if(index<4){
				divs.hide();//隐藏所有选中项内容
		...
		$(".header3-navcontent .tab6 li").bind("mouseenter", function () {
            var index = $(this).index();
			var divs = $(".header3-navcontent .content6 > div");
			$(this).parent().children("li").attr("class", "");
			$(this).attr("class", "on").show(); //设置当前选中项为选中样式
			divs.hide();//隐藏所有选中项内容
			divs.eq(index).show(); //显示选中项对应内容
