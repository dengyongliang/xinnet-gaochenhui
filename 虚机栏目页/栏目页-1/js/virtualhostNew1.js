...
$(function(){
	...
	lmyBtns.each(function(i){
		lmyBtns.eq(i).find(".virHost_part2_bot>a").each(function(j){
			...
			lmyBtns.eq(i).find(".virHost_part2_bot>a").eq(j).attr({"target":"_blank", "href":"http://www.xinnet.com/virtualhost/vhost-buy-detail.html?type=lx"+(i+1)+"&xh="+index+"&year=1"});
		})
	})
	...
	$(".zzfw_sel").click(function(){
		...
		if(selFath.hasClass("zzfwOn")){
			...
			$(".li_fwqd"+index).find(".virjgnew").html("150元");
			$(".virtualBuy_price_sp1").find("strong").html(allPrice()+"元");
			if(!$("li.zzfwOn").length){
				$(".fwqd_div").hide();
			}
	...
	// 购买年限点击
    $("#wzsj_sub,#sjksj_sub").click(function(){
    	...
    	qdspan.html(spanNum);
    	$(this).parents("li").next().find(".virjgnew").html(spanNum*150+"元");
    	$(".virtualBuy_price_sp1").find("strong").html(allPrice()+"元");
    })
    // 购买年限点击
    $("#wzsj_add,#sjksj_add").click(function(){
...
function allPrice(){
	...
	var jg_two = $(".virtualBuy_zzfw ul li").eq(0).hasClass("zzfwOn") ? $(".virjg2").html() : "0元";
	var jg_three = $(".virtualBuy_zzfw ul li").eq(1).hasClass("zzfwOn") ? $(".virjg3").html() : "0元";
	...
	var allprice = jg_one+jg_two+jg_three;
	return allprice;
}
